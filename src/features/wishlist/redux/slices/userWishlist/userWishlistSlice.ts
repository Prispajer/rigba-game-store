...
} from "./userWishlist.thunk";
import { UserWishlistState } from "./userWishlist.types";
...
const userWishListSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addUserProductToWishlistThunk.pending, (state) => {
        state.status = "Loading";
        state.error = null;
        state.message = "Loading...";
      })
      .addCase(addUserProductToWishlistThunk.fulfilled, (state, action) => {
        state.status = "Succeded";
        state.message = action.payload.message;
        state.products = action.payload.products || [];
      })
      .addCase(addUserProductToWishlistThunk.rejected, (state, action) => {
        state.status = "Failed";
        state.message = action.payload as string;
        state.error = action.payload as string;
        state.products = [];
      })
      .addCase(deleteUserProductFromWishlistThunk.pending, (state) => {
        state.status = "Loading";
        state.error = null;
        state.message = "Loading...";
      })
      .addCase(
        deleteUserProductFromWishlistThunk.fulfilled,
        (state, action) => {
      ...
      .addCase(deleteUserProductFromWishlistThunk.rejected, (state, action) => {
        state.status = "Failed";
        state.message = action.payload as string;
        state.error = action.payload as string;
      });
